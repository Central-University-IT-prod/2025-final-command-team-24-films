...
load_dotenv()
proxies = {
    "http://": "socks5://REDACTED",
    "https://": "socks5://REDACTED"
}
...
requests = httpx.Client(proxy="socks5://REDACTED", headers=headers)
...
def filter(film, release_date_low, release_date_high, genre_ids, watched, email):
    ...
    db.connect()
    db.initialize_tables()
    print(f'{watched} aaaaaaaaaaaaaaaaaaaaaaa')
    if watched != None:
        print(f'{watched} bbbbbbbbbbb')
    ...
    if genre_ids != None and "genre_ids" in c_res:
        ok = False
        for id in c_res["genre_ids"]:
            if str(id) not in genres_:
                continue
