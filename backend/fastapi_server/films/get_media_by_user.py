...
async def get_films_by_title(body:Search, search:str=None, token: str = Security(Bear)):
    ...
    for i in range(len(films)):
        if films[i]["media_id"] >= 0:
            film = await get_by_id(films[i]["media_id"], films[i]["media_type"], short=False)
            ...
            if films[i]["media_type"] == "movie":
                ...
                if search != None and fuzz.partial_ratio(film.name, search) < 75:
                    continue
                new_film = Film_to_front(title=film.name, poster_path=film.poster_path, overview=film.overview,
                                         release_date=film.first_air_date, id=films[i]["media_id"],
                                         watched=films[i]["watched"])
        else:
            if "genres_id" in body.model_dump():
                continue
